...
or issues. **Status (2026-09-08, issue #27):** `VERCEL_ORG_ID` + `VERCEL_PROJECT_ID` are set in Actions; **`VERCEL_TOKEN` is still missing** — continuous ship cannot complete the CLI path. Check names with `bash scripts/gh_secrets_names_check.sh` (never prints values). Until the token is set, every run
