# #103 — SDM CameraSound + Pub/Sub → Gemini detector → louder alarm

## Status

stubbed — SDM CameraSound → Pub/Sub → detector path documented

## Goal

SDM CameraSound + Pub/Sub → Gemini detector → louder alarm.
...
Live Pub/Sub subscription + SDM device events after owner enable.
