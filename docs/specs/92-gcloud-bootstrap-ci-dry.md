# #92 — gcloud bootstrap script + CI dry-check for Vertex/SDM/PubSub names

## Status

implemented — dry-check script + CI presence checks

## Goal

gcloud bootstrap script + CI dry-check for Vertex/SDM/PubSub names.
...
`--apply` only after explicit owner confirmation.
