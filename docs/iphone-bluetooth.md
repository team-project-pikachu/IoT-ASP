...
| Setting | Guidance |
|---------|----------|
| Unlock | Resume `AudioContext` on a user gesture (`Signal on` / Transmit) |
| Sample rate | Prefer **48 kHz** when the device exposes it (`AudioContext` options / observe `ctx.sampleRate`) |
| Mic capture | When listening: request `echoCancellation: false`, `noiseSuppression: false`, `autoGainControl: false` where the browser allows |
...
| Volume | Respect iOS absolute volume + in-app gain clamps (≤ ~8–12% residential default) |
...
| SensorKit | Research / entitlement path — parked |
