...
suite is itself proof the merge preserved #185's code verbatim. #186 (peak `0.60` -> `0.40`) is the
same shape and the same check: it changes one constant in `public/index.html` and the assertion that
pins it, so a half-applied merge would leave the two disagreeing. Confirmed they agree —
`public/index.html:732` and `tests/test_public_html.py:341` both read `0.40`.
