...
# Exit 0 on CLT smoke + SPM resolve (+ build when the toolchain allows).
...
# 2) SPM resolve + best-effort library build for Shared sources
(
  cd "$NATIVE"
  ...
  if swift build -c release >"$OUT" 2>"$ERR"; then
    ...
    echo "WARN: swift build skipped/failed on this host — smoke + resolve still OK"
    echo "      (full iOS/Watch schemes need Xcode.app — see docs/native-xcode.md)"
    head -n 20 "$ERR" || true
  fi
)
